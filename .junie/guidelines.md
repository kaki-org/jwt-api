...
1. **Ruby スタイルガイド**:
   - プロジェクトはコードスタイル強制のためにStandard Ruby (standardrb) を使用しています
   - ルールは Standard 固定で、cop ごとの設定変更はしません（standard-rails で Rails 用 cop も有効）
   - 対象 Ruby バージョンは `.tool-versions` から自動判定されます
   - `dip standardrb --fix` で自動整形できます
