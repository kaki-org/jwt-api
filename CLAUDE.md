...
- バックエンドのコマンドは dip 経由で実行する (dip rspec / dip standardrb / dip brakeman / dip rails c)。Docker外なら bundle exec。フロントエンドは front/ で pnpm。
...
- PRごとに standardrb + RSpec が走る (SimpleCov→Codecov)。デプロイは Heroku (heroku.yml)。
